...
export const PlanViewer: React.FC = () => {
  ...
  const renderInstructions = useCallback((instructions?: string[]) => {
    ...
    return (
      <ol className="list-decimal list-inside space-y-1.5 text-sm text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-800/40 p-3 rounded-xl border border-slate-100 dark:border-slate-800">
        {instructions.map((step, i) => <li key={i} className="leading-relaxed pl-1">{step}</li>)}
      </ol>
    );
  }, []);
  ...
  const handleApplySubstitution = useCallback((alt: AlternativeOption) => {
    ...
    if (subItem.type === 'Exercise') {
      ...
      if (idx !== -1) { day.workout[idx] = { ...day.workout[idx], name: alt.name, ...alt.data }; }
    } else {
      const idx = day.meals.findIndex(m => m.id === subItem.id);
      if (idx !== -1) { day.meals[idx] = { ...day.meals[idx], name: alt.name, ...alt.data }; }
    }
    setPlan(newPlan);
    if (isDraft) storageService.saveDraftPlan(newPlan); else storageService.updatePlan(newPlan);
  ...
  const isRestDay = !activeDay.workout || activeDay.workout.length === 0 || activeDay.workout[0].name.toLowerCase().includes('rest');
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-20">
      ...
      <div className="print:hidden">
            ...
            <div className="grid lg:grid-cols-2 gap-8 animate-in fade-in" key={activeDayIndex}>
                <div className="space-y-6">
                    ...
                    {isRestDay ? (
                      ...
                      activeDay.workout?.map((exercise: Exercise) => {
        ...
        {subModalOpen && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200"><div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col max-h-[85vh]"><div className="p-5 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center"><h3 className="font-bold text-lg text-slate-900 dark:text-white">Substitute Item</h3><button onClick={() => setSubModalOpen(false)}><X size={20} className="text-slate-400" /></button></div><div className="p-5 overflow-y-auto"><div className="flex gap-2 mb-4"><input type="text" value={subQuery} onChange={(e) => setSubQuery(e.target.value)} placeholder="Reason (e.g. Allergy, No Equipment)" className="flex-1 rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 p-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" /><button onClick={fetchAlternatives} disabled={!subQuery.trim() || subLoading} className="bg-indigo-600 text-white p-3 rounded-xl disabled:opacity-50">{subLoading ? <Loader2 className="animate-spin w-5 h-5" /> : <Shuffle className="w-5 h-5" />}</button></div><div className="space-y-3">{subOptions.map((opt, idx) => (<button key={idx} onClick={() => handleApplySubstitution(opt)} className="w-full text-left p-4 rounded-xl border border-slate-200 dark:border-slate-700 hover:border-indigo-500 bg-white dark:bg-slate-800 transition-all"><div className="font-bold text-slate-900 dark:text-white">{opt.name}</div><div className="text-xs text-slate-500 mt-1">{opt.reason}</div></button>))}</div></div></div></div>)}
      </div>
    </div>
  );
};